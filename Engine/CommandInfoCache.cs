...
namespace Microsoft.Windows.PowerShell.ScriptAnalyzer
{
    ...
    internal class CommandInfoCache
    {
...
#if DEBUG
            // do a bit of extra work to track cache hits and misses
            Lazy<CommandInfo> ci;
        ...
        {
            using ( var ps = System.Management.Automation.PowerShell.Create()) {
                // retrieve all the commands we have available by default
                foreach(CommandInfo ci in ps.Runspace.SessionStateProxy.InvokeCommand.GetCommands("*", CommandTypes.All, true)) {
                    // populate the cache with multiple keys as each on of these might be used to query
